...
class ResponseGenerator:
    ...
    def _init_system_prompt_templates(self):
        ...
        _core = (
            ...
            "**Format:** Markdown with **bold** for ALL financial figures (e.g. **$2.5B**, **+15%**, **42,000 employees**, **3.2x**), bullets, and tables. "
            "Use a markdown table whenever presenting 2 or more values across periods, companies, or metrics — never list them inline as prose. "
...
Instructions: Answer in 2-4 sentences with the specific number(s) or fact(s) requested. **Bold every financial figure** — dollar amounts, percentages, growth rates, counts (e.g. **$4.2B**, **+12%**, **95,000 employees**). MANDATORY: If the answer contains data across 2 or more periods or metrics, you MUST present it as a markdown table — no exceptions, no bullet lists. In prose, refer to companies by name or ticker WITHOUT a dollar sign (write "Datadog" or "DDOG", never "$DDOG"). CITATION EXAMPLES: If you see "SOURCE [55]" in the data, write [55] in your answer (WITH BRACKETS). If you see "SOURCE [13]", write [13]. ALWAYS include the brackets. DO NOT write 55 or 13 alone — NEVER use bare numbers as citations.{news_sources_instruction}{ten_k_sources_instruction}{transcript_sources_instruction} Attribute naturally (e.g. "According to {company_name}'s Q1 2025 earnings call [13]..."). Present findings first; if incomplete, add a short note after the table. Never say "chunks"—refer to company documents. Do not label the format ("here is a direct answer"). End with:\n\n**You might also ask:**\n- [Write a specific question using the actual company name and ticker, no $ prefix — related metric or trend]\n- [Write a specific question using the actual company name and ticker, no $ prefix — different analytical angle]\n- [Write a specific question using the actual company name and ticker, no $ prefix — deeper on a key finding]
{no_context_note}
...
Instructions: 3-5 sentences with key facts and figures. **Bold every financial figure** — dollar amounts, percentages, growth rates, counts (e.g. **$4.2B**, **+12%**, **95,000 employees**). MANDATORY: If the answer contains data across 2 or more periods or metrics, you MUST present it as a markdown table — no exceptions, no bullet lists. In prose, refer to companies by name or ticker WITHOUT a dollar sign (write "Datadog" or "DDOG", never "$DDOG"). CITATION EXAMPLES: If you see "SOURCE [55]" in the data, write [55] in your answer (WITH BRACKETS). If you see "SOURCE [13]", write [13]. ALWAYS include the brackets. DO NOT write 55 or 13 alone — NEVER use bare numbers as citations.{news_sources_instruction}{ten_k_sources_instruction}{transcript_sources_instruction} Present findings first; if incomplete, note briefly after the table. End with:\n\n**You might also ask:**\n- [Write a specific question using the actual company name and ticker, no $ prefix — related metric or trend]\n- [Write a specific question using the actual company name and ticker, no $ prefix — different analytical angle]\n- [Write a specific question using the actual company name and ticker, no $ prefix — deeper on a key finding]
{no_context_note}
...
Instructions: Start with a 1-2 sentence summary; use ## headers by topic; MANDATORY: any data across 2+ periods or metrics MUST be in a markdown table — never use bullet lists for multi-period data. **Bold every financial figure** — dollar amounts, percentages, growth rates, counts, ratios (e.g. **$4.2B**, **+12%**, **95,000 employees**, **2.3x**). In prose, refer to companies by name or ticker WITHOUT a dollar sign (write "Datadog" or "DDOG", never "$DDOG"). Be direct and conversational—avoid jargon and clichés ("Analyst Assessment", "Bottom Line"). CITATION EXAMPLES: If you see "SOURCE [55]" in the data, write [55] in your answer (WITH BRACKETS). If you see "SOURCE [13]", write [13]. ALWAYS include the brackets. DO NOT write 55 or 13 alone — NEVER use bare numbers as citations. EVERY fact needs a citation.{news_sources_instruction}{ten_k_sources_instruction}{transcript_sources_instruction} Reference {company_name} and period (e.g. "In Q1 2025 [13]..."). Present findings first; if incomplete, acknowledge briefly after the table. Never say "chunks." End with:\n\n**You might also ask:**\n- [Write a specific question using the actual company name and ticker, no $ prefix — related metric or trend]\n- [Write a specific question using the actual company name and ticker, no $ prefix — different analytical angle]\n- [Write a specific question using the actual company name and ticker, no $ prefix — deeper on a key finding]
{no_context_note}
