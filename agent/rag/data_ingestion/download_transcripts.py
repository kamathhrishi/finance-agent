...
class TranscriptDownloader:
    ...
    def download_transcript(self, ticker: str, year: int, quarter: int) -> Optional[Dict]:
        """Download earnings transcript for a specific ticker, year, and quarter"""
        try:
            api_url = f'https://api.api-ninjas.com/v1/earningstranscript?ticker={ticker}&year={year}&quarter={quarter}'
            
            headers = {'X-Api-Key': self.api_key}
            
            logger.info(f"Downloading transcript for {ticker} {year} Q{quarter}...")
            response = requests.get(api_url, headers=headers, timeout=30)

            if response.status_code == 200:
                data = response.json()
                if data:
                    logger.info(f"✅ Successfully downloaded transcript for {ticker} {year} Q{quarter}")
                    return {
                        'ticker': ticker,
                        'year': year,
                        'quarter': quarter,
                    ...
                    logger.info(f"📭 No transcript available for {ticker} {year} Q{quarter}")
                    return {
                        'ticker': ticker,
                        'year': year,
                        'quarter': quarter,
                ...
                logger.info(f"📭 No transcript found for {ticker} {year} Q{quarter} (404)")
                return {
                    'ticker': ticker,
                    'year': year,
                    'quarter': quarter,
                ...
                logger.warning(f"⚠️ Rate limit exceeded for {ticker} {year} Q{quarter} (429)")
                return {
                    'ticker': ticker,
                    'year': year,
                    'quarter': quarter,
                ...
                logger.error(f"❌ API error for {ticker} {year} Q{quarter}: {response.status_code} - {response.text}")
                return {
                    'ticker': ticker,
                    'year': year,
                    'quarter': quarter,
            ...
            logger.error(f"⏰ Timeout error for {ticker} {year} Q{quarter}")
            return {
                'ticker': ticker,
                'year': year,
                'quarter': quarter,
            ...
            logger.error(f"🌐 Network error for {ticker} {year} Q{quarter}: {e}")
            return {
                'ticker': ticker,
                'year': year,
                'quarter': quarter,
            ...
            logger.error(f"💥 Unexpected error for {ticker} {year} Q{quarter}: {e}")
            return {
                'ticker': ticker,
                'year': year,
                'quarter': quarter,
