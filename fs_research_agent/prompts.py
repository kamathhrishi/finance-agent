...
- ❌ **Citing scratchpad files** (`metadata.json`, `INDEX.md`, `_seen_accessions.json`, etc.) — these are agent-internal files used for navigation and date discovery, NOT narrative content. The reader sees `(filings/MSFT/10-K/FY2025/metadata.json:7-8)` as raw broken text. If you need to mention a filing's date or accession in the answer, write it as plain prose ("MSFT's latest 10-K, filed July 2025") with NO file-path citation. The ONLY citable files are `filing.md`, `sections/item-*.md`, and `exhibits/*.md` — never `.json`, never `INDEX.md`, never any `_*.json` state file.
